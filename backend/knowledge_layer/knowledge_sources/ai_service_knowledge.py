...
import os
from utils import get_first_paragraph, bytes_pretty_printer
...
def ai_services_overview(sim, knowledge_router, query_key, params):
    ...
    for task, ai_services in AI_SERVICE_TASK_MAP.items():
        ...
        for ai_service in ai_services:
            response += f"- {ai_service_name_mapper(ai_service['image_repository_url'])}\n"

    return response


# @knowledge_entry(
#     key="/ai_services?tasks={task_name}",
#     tags=[KnowledgeTag.AI_SERVICE],
#     related=[],
# )
# def ai_services_per_task_overview(sim, knowledge_router, query_key, params):
...
#     if task_name not in AI_SERVICE_TASK_MAP:
#         return f"Task {task_name} is currently not supported. Please check the supported tasks by querying /ai_services"

#     response = f"There are in total {len(AI_SERVICE_TASK_MAP[task_name])} AI services for task {task_name}:"

#     for ai_service in AI_SERVICE_TASK_MAP[task_name]:
#         response += f"""
# ---
# AI Model name: {ai_service_name_mapper(ai_service['image_repository_url'])}
...
# Description (short): {get_first_paragraph(ai_service['task_detail'])}
# """
#     return response
