...
import xApps  # import the package itself

def load_all_xapps():
    xapp_classes = []

    # Iterate through all modules in the xApps package
    for _, module_name, is_pkg in pkgutil.iter_modules(xApps.__path__):
        if is_pkg or module_name == "xapp_base":
            continue  # skip subpackages and base class

        # Dynamically import the module
        module = importlib.import_module(f"xApps.{module_name}")
