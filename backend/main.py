...
async def websocket_handler(websocket):
    ...
    while True:
        ...
        if message == "start_simulation":
            ...
            await websocket.send(json.dumps({"type": "simulation_state", "data": simulation_engine.to_json()}))
        else:
            print("Unknown command")

async def main():
    async with websockets.serve(websocket_handler, settings.WS_SERVER_HOST, settings.WS_SERVER_PORT):
        print(f"WebSocket server started on ws://{settings.WS_SERVER_HOST}:{settings.WS_SERVER_PORT}")
