...
import math
import random
from backend.core_network import CoreNetwork
from backend.ric import RanIntelligentController
import settings
...
class SimulationEngine:
    ...
    def network_setup(self):
        ...
        for i, ru_data in enumerate(settings.RAN_DEFAULT_BS_LIST):
            ru = RU(simulation_engine=self.simulation_engine, ru_id=ru_data[0], position_x=ru_data[1], position_y=ru_data[2])
            self.RU_list.add(ru)
        ...
        # self.base_station = BaseStation(self)
        # self.base_station.init_RU_list(settings.RAN_BS_DEFAULT_LIST)
    ...
    def spawn_random_ue(self):
        position_x = random.randint(settings.UE_BOUNDARY_X_MIN, settings.UE_BOUNDARY_X_MAX)
        position_y = random.randint(settings.UE_BOUNDARY_Y_MIN, settings.UE_BOUNDARY_Y_MAX)
        min_prb = random.randint(settings.UE_MIN_PRB_RANGE_MIN, settings.UE_MIN_PRB_RANGE_MAX)
    ...
    def spawn_UEs(self, fixed_count=settings.UE_FIXED_COUNT):
        if not fixed_count:
            ue_to_spawn = math.floor(random.randint(settings.UE_DEFAULT_SPAWN_RATE_MIN, settings.UE_DEFAULT_SPAWN_RATE_MAX))
            ...
            number_of_ue_to_refresh = math.floor(len(self.UE_list) * settings.UE_REFRESH_PORTION)
            print("number of UEs " + str(len(self.UE_list)), "number of UEs to refresh " + str(number_of_ue_to_refresh))

            if number_of_ue_to_refresh > 0:
                # remove some UEs
                UE_to_remove_list = random.sample(list(self.UE_list), number_of_ue_to_refresh)
    ...
    def update_RUs(self, delta_time):
        ...
        for ru in self.base_station.RU_list:
            ...
            if ru.demanded_prb > ru.allocable_prb:
                ...
                ru.allocable_prb = max(settings.RAN_BS_DEFAULT_MIN_ALLOCABLE_PRB, ru.allocable_prb)
            ...
            for ue in ru.served_UE_list:
                ...
                prb_to_allocate_to_ue = min(ue.min_prb - ue.allocated_prb, ru.allocable_prb - ru.allocated_prb)
            ...
            if settings.RIC_ENABLE_HANDOVER and ru.demanded_prb > ru.max_prb:
                if ru.allocated_prb == ru.max_prb and ru.allocable_prb == ru.allocated_prb:
                    ...
                    for ue in ru.served_UE_list:
                        ...
                        if settings.RIC_BRUTAL_HANDOVER:
                            available_neighbour_RUs = [neighbour_ru for neighbour_ru in ru.neighbour_RU_list if neighbour_ru in ue.connected_RU_list]
                            ...
                            for neighbour_ru in ru.neighbour_RU_list:
                                ...
                                if (neighbour_ru.demanded_prb + ue.min_prb) <= neighbour_ru.max_prb:
                                    self.handover_action_list.append((ue, ru, neighbour_ru))
                                    # # perform the handover
                                    # # mark the ue to be removed later
                                    # ue_to_handover_list.append(ue)
    ...
    def update(self, delta_time):
        ...
        for ru in self.base_station.RU_list:
            ...
            if ru.ru_id == "ru_11":
                print(f"RU {ru.ru_id} current load: {ru.current_load} demanded prb: {ru.demanded_prb} allocated prb: {ru.allocated_prb}")
    ...
    async def start(self):
        ...
        while True:
            ...
            print("\n-------------------------\n")
            await self.websocket.send(json.dumps({"type": "simulationsimulation_state", "data": self.to_json()}))
    ...
    def to_json(self):
        return {
            ...
            "base_station": self.base_station.to_json(),
